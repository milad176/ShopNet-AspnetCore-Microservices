...
namespace Discount.API.Extensions
{
    public static class WebHostExtensions
    {
        public static IServiceCollection MigrateDatabase<TContext>(this IServiceCollection serviceCollection)
        {
            ...
            try
            {
                logger.LogInformation("Migrate postgresql database started.");

                var policy = Policy.Handle<SocketException>()
                   .Or<NpgsqlException>()
                   .WaitAndRetry(retryCount, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)), (ex, time) =>
                   {
                       logger.LogError(ex, "An error occurred while migrating the postgresql database, failed trying after {TimeOut}s", $"{time.TotalSeconds:n1}");
                   });
